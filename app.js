...
function sortListings(items) {
  ...
  sorted.sort((a, b) => {
    if (mode === 'price-low') return (Number(a.price) || Number.POSITIVE_INFINITY) - (Number(b.price) || Number.POSITIVE_INFINITY)
...
createListingBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    if (!editingId && selectedFiles.length > 8) return showFormError('Please choose no more than 8 images.', imageEl)
...
sortEl?.addEventListener('change', () => renderFilteredListings())
...
function renderFilteredListings() {
  ...
  const sortedFiltered = sortListings(filtered)
  listCount.textContent = `${sortedFiltered.length} listing${sortedFiltered.length === 1 ? '' : 's'}`
  ...
  sortedFiltered.forEach((item) => renderListing(item, listingsContainer))
...
async function runAiSearch(queryText) {
  ...
  sortListings(results).forEach((item) => renderListing(item, listingsContainer))
