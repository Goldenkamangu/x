...
function openStore(userId) {
  ...
  const s = storesById[String(userId)]
...
function openStoreManage() {
  ...
  const mine = storesById[String(currentUser.id)]
  ...
  storeManageIntro.textContent = mine?.name
    ...
    : "Open your own store inside LinkHub — a page with your name, a short bio, a banner, and all your listings in one place you can share."
...
async function saveStoreManage(event) {
  ...
  let banner_url = storesById[String(currentUser.id)]?.banner_url || null
  ...
  if (bannerFile) {
    try {
      ...
      const { data: upData, error: upErr } = await storage.upload(path, compressed, { upsert: false })
      if (!upErr) {
        const { data: pub } = storage.getPublicUrl(upData?.path || path)
  ...
  try {
    const { error: storeErr } = await db.from('stores').upsert({ id: currentUser.id, name, bio, banner_url })
    if (storeErr) throw storeErr
    storesById[String(currentUser.id)] = { id: currentUser.id, name, bio, banner_url }
    storeManageMsg.textContent = 'Your store is live.'
    storeManageViewBtn.style.display = ''
    storeManageHeading.textContent = 'My Store'
    // Refresh the nav pill label and any open feed cards that reference this store.
    await handleAuthChange()
    renderFilteredListings()
  } catch (e) {
    console.warn('Saving store failed (has the stores table been created?)', e)
    storeManageMsg.textContent = 'Could not save your store — check that the "stores" table has been created in Supabase.'
  }
}
...
async function runListingCleanup() {
  ...
  try {
    await db.from('listings').delete().in('id', toDelete)
...
async function fetchAndRenderListings() {
  ...
  try {
    ...
    try {
      ...
      if (!storeErr) storesById = Object.fromEntries((storeRows || []).map((s) => [String(s.id), s]))
    } catch (e) {
