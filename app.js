...
function setMenuAuthState() {
  const signedIn = !!currentUser
  if (menuStatus) {
    if (signedIn) {
      const raw = currentUser.user_metadata?.full_name || getDisplayNameFromEmail(currentUser.email) || 'User'
      const name = raw.trim().split(/\s+/).filter(Boolean).map(part => part.charAt(0).toUpperCase() + part.slice(1).toLowerCase()).join(' ')
      menuStatus.textContent = `Signed in as ${name}`
    } else {
      menuStatus.textContent = 'Not signed in'
    }
  }
  document.querySelectorAll('.menu-item-auth').forEach(el => { el.style.display = signedIn ? '' : 'none' })
  document.querySelectorAll('.menu-item-guest').forEach(el => { el.style.display = signedIn ? 'none' : '' })
}
...
document.addEventListener('keydown', (event) => {
  if (event.key === 'Escape' && siteMenu?.classList.contains('open')) closeSiteMenu()
})
...
document.querySelectorAll('.menu-item').forEach(item => {
  item.addEventListener('click', async () => {
    ...
    if (action === 'browse') {
      ...
      if (currentUser) openMyListings()
    } else if (action === 'account') {
      if (currentUser) openAccountSettings()
...
function showInstallBanner(force = false) {
  ...
  if (!force && shownCount >= INSTALL_PROMPT_MAX_SHOWS) return
  if (!force) localStorage.setItem('install-prompt-shown-count', String(shownCount + 1))
