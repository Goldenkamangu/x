...
const listingMsg =
  document.getElementById('listing-msg')
...
async function handleAuthChange() {
  ...
  if (user) {
    ...
    authArea.innerHTML =
      `<div class="auth-pill-group"><span class="auth-pill">Welcome, ${escapeHtml(displayName)}</span><button id="btn-my-listings" class="auth-pill auth-my-listings" type="button">My Listings</button><button id="btn-logout" class="auth-pill auth-logout" type="button">Logout</button></div>`
...
createListingBtn.addEventListener(
  ...
  async () => {
    ...
    try {
      ...
      // If editing, perform update flow
      if (editingId) {
        const updateObj = {
          ...obj
        }

        if (!updateObj.contact_method) {
          delete updateObj.contact_method
        }

        if (!updateObj.contact_details) {
...
function renderFilteredListings() {
  ...
  /*
    FIX:
    showMineOnly was not defined.
    The normal search now uses a
    simple empty-results message.
  */
  if (!filtered.length) {
    listingsContainer.innerHTML =
...
function renderListing(
  ...
  parts.push(
    `<h3 class="listing-title">${escapeHtml(
      l.title
    )}</h3>`
  )
  ...
  if (l.price) {
    try {
      ...
      parts.push(
        `<div class="listing-price">${escapeHtml(
          display
        )}</div>`
      )
  ...
  if (l.delivery_type) {
    meta.push(
      `<div><strong>Delivery:</strong> ${escapeHtml(
        l.delivery_type
      )}</div>`
    )
  }
  ...
  if (l.url) {
    meta.push(
      `<div><strong>Condition:</strong> ${escapeHtml(
        l.url
      )}</div>`
    )
  }
