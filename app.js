    ...
    if (table === 'listings') {
      return {
        ...
        update(values) {
          return {
            async eq(column, value) {
              if (column !== 'id') return { error: { message: 'Local fallback only supports id updates.' } }
              const index = localState.listings.findIndex((row) => String(row.id) === String(value))
              if (index === -1) return { error: { message: 'Listing not found.' } }
              localState.listings[index] = { ...localState.listings[index], ...values }
              persistLocalState()
              return { error: null }
            }
          }
        ...
        delete() {
          return {
            async eq(column, value) {
              if (column !== 'id') return { error: { message: 'Local fallback only supports id deletes.' } }
              const before = localState.listings.length
              localState.listings = localState.listings.filter((row) => String(row.id) !== String(value))
              if (localState.listings.length === before) return { error: { message: 'Listing not found.' } }
              persistLocalState()
              return { error: null }
            }
          }
...
// --- Overlay open/close helpers (Auth + Create Listing) ---
const authOverlay = document.getElementById('auth-overlay')
const authOverlayClose = document.getElementById('auth-overlay-close')
const createListingOverlay = document.getElementById('create-listing-overlay')
const createListingClose = document.getElementById('create-listing-close')
...
authOverlayClose?.addEventListener('click', closeAuthOverlay)
authOverlay?.addEventListener('click', (ev) => {
  if (ev.target === authOverlay) closeAuthOverlay()
})

function openCreateListingOverlay() {
  if (!createListingOverlay) return
  createListingOverlay.classList.remove('hidden')
  createListingOverlay.setAttribute('aria-hidden', 'false')
}
function closeCreateListingOverlay() {
  if (!createListingOverlay) return
  createListingOverlay.classList.add('hidden')
  createListingOverlay.setAttribute('aria-hidden', 'true')
}
createListingClose?.addEventListener('click', closeCreateListingOverlay)
createListingOverlay?.addEventListener('click', (ev) => {
  if (ev.target === createListingOverlay) closeCreateListingOverlay()
})
document.addEventListener('keydown', (ev) => {
  if (ev.key !== 'Escape') return
  closeAuthOverlay()
  closeCreateListingOverlay()
})
...
async function handleAuthChange() {
  ...
  if (user) {
    ...
    authArea.innerHTML = `<div class="auth-pill-group"><span class="auth-pill">Welcome, ${escapeHtml(displayName)}</span><button id="btn-new-listing" class="auth-pill auth-new-listing" type="button">+ New Listing</button><button id="btn-my-listings" class="auth-pill auth-my-listings" type="button">My Listings</button><button id="btn-logout" class="auth-pill auth-logout" type="button">Logout</button></div>`
    ...
    document.getElementById('btn-new-listing').addEventListener('click', () => {
      ...
      openCreateListingOverlay()
    })
  } else {
    authArea.innerHTML = `<button id="btn-sign-in" class="auth-pill auth-signin" type="button">Sign In</button>`
    document.getElementById('btn-sign-in').addEventListener('click', openAuthOverlay)
    closeCreateListingOverlay()
  }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (loadingScreen) setTimeout(() => loadingScreen.classList.add('hidden'), 1400)
...
async function fetchAndRenderListings() {
  ...
  try {
    ...
    if (listingsContainer) listingsContainer.innerHTML = `<div class="muted">${escapeHtml(err.message || 'Failed to load listings.')}</div>`
  }
}
...
function renderFilteredListings() {
  ...
  if (listCount) listCount.textContent = `${filtered.length} listing${filtered.length === 1 ? '' : 's'}`
  if (!listingsContainer) return 0
  listingsContainer.innerHTML = ''
  if (!filtered.length) {
...
async function runAiSearch(queryText) {
  ...
  if (!listingsContainer) return { type: 'search', count: 0, displayTerm }
  listingsContainer.innerHTML = ''
  if (!results.length) {
    if (listCount) listCount.textContent = '0 listings'
    return { type: 'search', count: 0, displayTerm }
  }

  if (listCount) listCount.textContent = `${results.length} listing${results.length === 1 ? '' : 's'}`
