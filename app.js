...
const favoritedIds = new Set(JSON.parse(localStorage.getItem('favorited-listings') || '[]'))
function persistFavorites() {
  localStorage.setItem('favorited-listings', JSON.stringify([...favoritedIds]))
}
...
createListingBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    let image_url = null
    let file = imageEl.files && imageEl.files[0]
    if (file) {
      listingMsg.textContent = 'Compressing image…'
    ...
    // Validate the information needed to publish a listing.
    // The button is type="button", so browser `required` validation does not run automatically.
    if (!obj.title) {
      listingMsg.textContent = 'Please enter a title for your listing.'
      titleEl.focus()
      return
    }

    if (!obj.price) {
      listingMsg.textContent = 'Please enter a price for your listing.'
      priceEl.focus()
      return
    }

    // If a contact method is selected, the matching contact details are required.
    if (obj.contact_method && !obj.contact_details) {
      listingMsg.textContent = 'Please enter your contact details so buyers know how to reach you.'
      contactDetailsEl.focus()
      return
    }

    // If contact details are entered without choosing a method, ask the user to choose one.
    if (!obj.contact_method && obj.contact_details) {
      listingMsg.textContent = 'Please choose a contact method for the contact details you entered.'
      contactMethodEl.focus()
      return
    }
...
function renderFilteredListings() {
  ...
  filtered.forEach((item) => renderListing(item))
...
async function runAiSearch(queryText) {
  ...
  results.forEach((item) => renderListing(item))
