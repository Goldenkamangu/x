...
async function saveStoreManage(event) {
  ...
  if (bannerFile) {
    try {
      const compressed = await compressImage(bannerFile, 1600, 0.8)
      const path = `${currentUser.id}/store-banner_${Date.now()}_${compressed.name}`
      const storage = db.storage.from('listing-images')
      const { data: upData, error: upErr } = await storage.upload(path, compressed, { upsert: false })
      if (!upErr) {
        uploadedBannerPath = upData?.path || path
        const { data: pub } = storage.getPublicUrl(uploadedBannerPath)
        if (pub?.publicUrl) banner_url = pub.publicUrl
      }
    } catch (e) {
      console.warn('Store banner upload failed', e)
    }
  }
  try {
    let payload = { id: currentUser.id, name, bio, banner_url }
    ...
    if (storeErr && /column.*id|not.?null.*id|user_id|duplicate key/i.test(storeErr.message || '')) {
      payload = { user_id: currentUser.id, name, bio, banner_url }
    ...
    if (uploadedBannerPath && existingStore?.banner_url && existingStore.banner_url !== banner_url) {
      const oldPaths = extractStoragePaths({ image_url: existingStore.banner_url })
      if (oldPaths.length) {
        try { await db.storage.from('listing-images').remove(oldPaths) }
        catch (cleanupErr) { console.warn('Could not remove replaced store banner', cleanupErr) }
      }
    }
    ...
    if (uploadedBannerPath) {
      try { await db.storage.from('listing-images').remove([uploadedBannerPath]) }
      catch (cleanupErr) { console.warn('Could not clean up store banner after save failure', cleanupErr) }
    }
...
async function deleteMyStore() {
  ...
  try {
    if (existingStore.banner_url) {
      const paths = extractStoragePaths({ image_url: existingStore.banner_url })
      bannerPath = paths[0] || null
      if (bannerPath) {
        try {
          const response = await fetch(existingStore.banner_url, { cache: 'no-store' })
          if (response.ok) bannerBlob = await response.blob()
        } catch (fetchErr) {
          console.warn('Could not snapshot store banner before deletion', fetchErr)
        }
      }
    }

    // Remove the Storage object first so the store cannot be deleted while
    // knowingly leaving its banner behind. If the DB delete later fails,
    // restore the banner from the snapshot when possible.
    if (bannerPath) {
      const { error: storageErr } = await db.storage.from('listing-images').remove([bannerPath])
      if (storageErr) throw storageErr
    }

    const { error: deleteErr } = await db.from('stores').delete().eq('id', currentUser.id)
    ...
    // If the database deletion failed after the banner was removed, restore it.
    if (bannerPath && bannerBlob) {
      try {
        const { error: restoreErr } = await db.storage.from('listing-images').upload(bannerPath, bannerBlob, { upsert: true, contentType: bannerBlob.type || 'image/jpeg' })
        if (restoreErr) console.warn('Could not restore store banner after failed deletion', restoreErr)
      } catch (restoreErr) {
        console.warn('Could not restore store banner after failed deletion', restoreErr)
      }
    }
    const message = e?.message || 'Unknown Supabase error'
...
// Small talk is handled locally, without calling the AI edge function at
// all — guarantees greetings, thanks, and basic chit-chat never get misread
// as a product search, regardless of how (or whether) the server responds.
const CARTY_SMALLTALK_RE = /^(hi|hello|hey|yo|sup|howzit|good\s?(morning|afternoon|evening|night)|thanks?|thank\s?you|cool|ok(ay)?|nice|lol|haha|bye|goodbye|see\s?ya|later|how\s?are\s?you|how'?s?\s?it\s?going|what'?s?\s?up|who\s?are\s?you|what\s?are\s?you|what\s?can\s?you\s?do|are\s?you\s?(a\s?)?(bot|real|human|ai))\b[\s!.,?]*(carty)?[\s!.,?]*$/i
const CARTY_SMALLTALK_REPLIES = [
  "Hey! 👋 What are you looking for today?",
  "Hi there! Tell me what you're shopping for and I'll search LinkHub for it.",
  "Hey! I'm here — try something like \"phone under R2000\" and I'll find it for you.",
  "I'm doing great, thanks for asking! I help you find things people are selling here — what do you need?",
  "I'm Carty 🛒 — LinkHub's shopping assistant. Tell me what you're after and I'll search for it."
]

// A rough, local guess at whether a message looks like a product search —
// used only when the AI edge function is unreachable, so we don't run a
// keyword search (and show a confusing "no listings found") on plain
// conversation just because the network call failed.
function looksLikeProductQuery(text) {
  const t = text.toLowerCase()
  if (/\d/.test(t)) return true
  if (/(under|below|less than|more than|around|about|cheap|affordable|budget|for sale|selling|sell|looking for|need a|need an|want a|want an|buy|price|available)\b/.test(t)) return true
  if (/(phone|iphone|samsung|laptop|macbook|computer|tv|television|fridge|freezer|couch|sofa|car|bakkie|bike|bicycle|scooter|shoe|sneaker|dress|clothes|table|desk|chair|speaker|headphone|earphone|console|playstation|ps4|ps5|xbox|camera|watch|jacket|jeans|book|furniture|appliance)\b/.test(t)) return true
  return false
}
...
async function runAiSearch(queryText) {
  ...
  if (CARTY_SMALLTALK_RE.test(term)) {
    const reply = CARTY_SMALLTALK_REPLIES[Math.floor(Math.random() * CARTY_SMALLTALK_REPLIES.length)]
  ...
  try {
    ...
    console.warn('AI search request failed, falling back to local matching:', err)
    aiUnavailable = true
  }

  // If the AI classifier couldn't be reached and this doesn't look like a
  // product search, don't silently run a keyword search on plain
  // conversation — that just produces a confusing "no listings found" for
  // something the user never meant as a search.
  if (aiUnavailable && !looksLikeProductQuery(term)) {
    return { type: 'chat', reply: "I didn't quite catch that — could you tell me what you're shopping for? Something like \"phone under R2000\" works great." }
  }
