...
async function runAiSearch(queryText) {
  ...
  if (!term) {
    const count = renderFilteredListings() || 0
    return { count, displayTerm: '' }
  }
  ...
  try {
    ...
    results = applyAiFilters(base, filters)
  } catch (err) {
    console.warn('AI search request failed, falling back to keyword matching:', err)
  }

  // If the AI path found nothing (or the request itself failed), fall back
  // to matching individual meaningful words from what was actually typed.
  if (!results.length) {
...
cartyForm?.addEventListener('submit', async (ev) => {
  ...
  if (cartyMessage) cartyMessage.textContent = `Looking for "${term}"…`
  ...
  if (cartyMessage) {
    if (result.count > 0) {
      ...
      cartyMessage.textContent = `There's currently no one selling "${result.displayTerm}" at the moment. Try checking back later, or search for something else!`
    }
  }
