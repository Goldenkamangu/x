...
// Listings narrowed by the active category chip and the "My Listings" toggle —
// used as the starting point for both plain search and Carty.
function getScopedListings() {
  ...
  if (showMineOnly && currentUser) {
    base = base.filter((item) => item.user_id === currentUser.id)
  }
  return base
}
...
async function handleAuthChange() {
  ...
  if (user) {
    ...
    authArea.innerHTML = `<div class="auth-pill-group"><span class="auth-pill">Welcome, ${escapeHtml(displayName)}</span><button id="btn-my-listings" class="auth-pill auth-my-listings${showMineOnly ? ' active' : ''}" type="button">${showMineOnly ? 'All Listings' : 'My Listings'}</button><button id="btn-logout" class="auth-pill auth-logout" type="button">Logout</button></div>`
    ...
    document.getElementById('btn-my-listings').addEventListener('click', () => {
      ...
      const btn = document.getElementById('btn-my-listings')
      if (btn) {
        btn.textContent = showMineOnly ? 'All Listings' : 'My Listings'
